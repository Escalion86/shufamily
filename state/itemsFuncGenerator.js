...
const messages = {}
const msg = {
  request: ['а', 'заявка', 'заявку'],
  event: ['о', 'мероприятие'],
  costumer: ['', 'клиент', 'клиента'],
}

for (const [key, arrText] of Object.entries(msg)) {
  messages[key] = {
    update: {
      success: `${capitalizeFirstLetter(arrText[1])} обновлен${arrText[0]}`,
      error: `Не удалось обновить ${arrText[2] ?? arrText[1]}`,
    },
    add: {
      success: `${capitalizeFirstLetter(arrText[1])} создан${arrText[0]}`,
      error: `Не удалось создать ${arrText[2] ?? arrText[1]}`,
    },
    delete: {
      success: `${capitalizeFirstLetter(arrText[1])} удален${arrText[0]}`,
      error: `Не удалось удалить ${arrText[2] ?? arrText[1]}`,
    },
  }
}
